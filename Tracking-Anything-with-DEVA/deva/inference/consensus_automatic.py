...
def solve_with_gurobi(pairwise_iou: np.ndarray, pairwise_f1: np.ndarray, pairwise_iou_indicator: np.ndarray,
    ...
    #Ours- we've removed the *2 by replacing it with the f_score.
    m.setObjective(
        # high support, *2 to compensate because we only computed the upper triangle
        (pairwise_iou @ x).sum() + (pairwise_f1 @ x).sum()
        # few segments -- the paper says *0.5 but it's later found
        # that the experiments were done with alpha=1 -- should not have a major impact
        - x.sum(),
        GRB.MAXIMIZE)
...
def solve_with_pulp(pairwise_iou: np.ndarray, pairwise_f1: np.ndarray, pairwise_iou_indicator: np.ndarray,
                    total_segments: int) -> List[bool]:
    ...
    support_objective = pulp.LpAffineExpression([(x[i], pairwise_iou[:, i].sum() + pairwise_f1[:, i].sum())
                                                 for i in range(total_segments)])
...
def find_consensus_auto_association(frames: List[FrameInfo],
    ...
    for f in frames:
        ...
        #Debugging code
        # print(image.shape)
        # if(time_indices[0] > 00):
        #     cpu_mask = f.mask.cpu().detach().numpy().astype(dtype=np.uint8)
        #     cpu_mask *= int(255/max(1, np.max(cpu_mask)))
        #     cpu_mask = np.expand_dims(cpu_mask, 2)
        #     cpu_mask = cv2.applyColorMap(cpu_mask, cv2.COLORMAP_JET)
        #     cv2.imshow(f"{time_indices[ind2]}.{ind2}", cpu_mask)
        #     ind2+=1
        # diffs.append(cpu_mask)
    ...
    # print(len(time_indices), len(segments_info))
    ...
    for i, this_seg_info in enumerate(segments_info):
        ...
        for si, seg_info in enumerate(this_seg_info):
            ...
            # [x, y] = _get_centroid(masks[i], old_id)
            # new_seg_info.set_centroid(x, y)
    ...
    for ti, image, mask, mapping in zip(time_indices, images, masks, channel_to_id_mappings):
        ...
        if ti == keyframe_ti:
            ...
            projected_mask = torch.cat([torch.ones_like(mask[0:1]) * 0.5, mask],
                                       dim=0)
        else:
            # print(image.shape, mask.shape)
        ...
        #Debugging code
        # cpu_mask = projected_mask.cpu().detach().numpy().astype(dtype=np.uint8)
        # cpu_mask *= int(255/max(1, np.max(cpu_mask)))
        # # cpu_mask = cv2.absdiff(cpu_mask, diffs[index])
        # # cpu_mask = np.expand_dims(cpu_mask, 2)
        # cpu_mask = cv2.applyColorMap(cpu_mask, cv2.COLORMAP_JET)
        # cv2.imshow(f"{ti}.{index}", cpu_mask)
    ...
    pairwise_f1 = np.zeros((total_segments, total_segments), dtype=np.float32)
    ...
    if total_segments == 0:
        ...
        output_info = []
        # print("No segments passed to consensus", flush=True)
                ...
                for obj1 in frame_index_to_seg_info[i]:
                    ...
                    for obj2 in frame_index_to_seg_info[j]:
                        ...
                        if iou > 0.5:
                            ...
                            pairwise_f1[mask1_id - 1, mask2_id - 1] = iou
                            break

    # make symmetric
    pairwise_iou = pairwise_iou + pairwise_iou.T
    pairwise_f1 = pairwise_f1 + pairwise_f1.T
    ...
    pairwise_f1 = pairwise_f1 * pairwise_iou_indicator
    segments_area /= image_area  # normalization

    if use_gurobi:
        try:
            results = solve_with_gurobi(pairwise_iou, pairwise_f1, pairwise_iou_indicator, total_segments)
        except gp.GurobiError as e:
            print(e)
            print('GurobiError, falling back to pulp')
            use_gurobi = False
    if not use_gurobi:
        results = solve_with_pulp(pairwise_iou, pairwise_f1, pairwise_iou_indicator, total_segments)
    ...
    for channel_id, selected in enumerate(results):
        if selected:
            ...
            # [x, y] = new_object_info.get_centroid()
    ...
    #Debugging code
    # if(keyframe_ti > 0):
    #     cpu_mask = output_mask.cpu().detach().numpy().astype(dtype=np.uint8)
    #     cpu_mask *= int(255/max(1, np.max(cpu_mask)))
    #     cpu_mask = np.expand_dims(cpu_mask, 2)
    #     cpu_mask = cv2.applyColorMap(cpu_mask, cv2.COLORMAP_JET)
    #     cv2.imshow("Output", cpu_mask)
    #     cv2.waitKey()
    #     cv2.destroyAllWindows()
    # print(output_mask.max())
    # print(time_indices, len(sorted_by_area), flush=True)
    return keyframe_ti, output_mask, output_info
